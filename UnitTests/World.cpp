...
TEST(World, Recreate)
{
    ...
    for (const auto &entry: worldContactMap) {
        ...
        if (it != recreatedContactMap.end()) {
            ...
            ASSERT_NE(recreatedIdxA, DynamicTree::InvalidSize) << os.str();
            ASSERT_NE(recreatedIdxB, DynamicTree::InvalidSize) << os.str();
            ASSERT_NE(worldIdxA, DynamicTree::InvalidSize) << os.str();
            ASSERT_NE(worldIdxB, DynamicTree::InvalidSize) << os.str();
