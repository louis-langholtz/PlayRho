...
class UnitVec
{
  public:
    ...
    template <typename T> using PolarCoord = std::enable_if_t<IsArithmetic<T>::value, std::pair<UnitVec, T>>;
...
template <std::size_t I> constexpr UnitVec::value_type get(UnitVec v) noexcept
{
...
template <> constexpr UnitVec::value_type get<0>(UnitVec v) noexcept
{
    return v.GetX();
}

/// @brief Gets element 1 of the given collection.
template <> constexpr UnitVec::value_type get<1>(UnitVec v) noexcept
{
    return v.GetY();
}
...
template <> constexpr d2::UnitVec GetInvalid() noexcept
{
    return d2::UnitVec{};
}
...
template <> class tuple_size<playrho::d2::UnitVec> : public std::integral_constant<std::size_t, 2>
{
};

/// @brief Tuple element type info for <code>playrho::d2::UnitVec</code>.
template <std::size_t I> class tuple_element<I, playrho::d2::UnitVec>
{
  public:
