...
class Sweep
{
  public:
    ...
    /// @brief Initializing constructor.
    constexpr explicit Sweep(const Position p, const Length2 lc = Length2{0_m, 0_m}) : Sweep{p, p, lc, 0}
    {
        // Intentionally empty.
    }
...
template <> constexpr bool IsValid(const d2::Sweep &value) noexcept
{
    return IsValid(value.pos0) && IsValid(value.pos1) && IsValid(value.GetLocalCenter()) && IsValid(value.GetAlpha0());
}
