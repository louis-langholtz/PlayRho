...
template <typename T> constexpr auto &GetX(T &value)
{
    return get<0>(value);
}

/// @brief Gets the "Y" element of the given value - i.e. the second element.
template <typename T> constexpr auto &GetY(T &value)
{
    return get<1>(value);
}

/// @brief Gets the "Z" element of the given value - i.e. the third element.
template <typename T> constexpr auto &GetZ(T &value)
{
    return get<2>(value);
}

/// @brief Gets the "X" element of the given value - i.e. the first element.
template <typename T> constexpr auto GetX(const T &value)
{
    return get<0>(value);
}

/// @brief Gets the "Y" element of the given value - i.e. the second element.
template <typename T> constexpr auto GetY(const T &value)
{
    return get<1>(value);
}

/// @brief Gets the "Z" element of the given value - i.e. the third element.
template <typename T> constexpr auto GetZ(const T &value)
{
    return get<2>(value);
}
...
template <typename T> constexpr auto StripUnit(const T &v) -> decltype(StripUnit(v.get()))
{
...
template <typename T, typename U> constexpr U Secant(T target, U a1, T s1, U a2, T s2) noexcept
{
...
template <typename T> constexpr T Bisect(T a1, T a2) noexcept
{
    return (a1 + a2) / 2;
}
...
template <typename T> constexpr bool IsOdd(T val) noexcept
{
...
template <class TYPE> constexpr auto Square(TYPE t) noexcept
{
    return t * t;
}
...
template <typename T> inline auto Atan2(T y, T x)
{
...
template <typename T,
          typename = std::enable_if_t<IsIterable<T>::value && IsAddable<decltype(*begin(std::declval<T>()))>::value>>
...
template <typename T> inline std::enable_if_t<IsArithmetic<T>::value, T> RoundOff(T value, unsigned precision = 100000)
{
...
/// @relatedalso Vector
template <typename T, std::size_t N> constexpr Vector<T, N> abs(const Vector<T, N> &v) noexcept
{
...
template <typename T> constexpr std::enable_if_t<std::is_arithmetic<T>::value, bool> AlmostZero(T value)
{
...
constexpr std::enable_if_t<std::is_floating_point<T>::value, bool> AlmostEqual(T x, T y, int ulp = 2)
{
    ...
    //
    return (abs(x - y) < (std::numeric_limits<T>::epsilon() * abs(x + y) * static_cast<T>(ulp))) || AlmostZero(x - y);
}
...
/// @see ModuloViaTrunc
template <typename T> inline auto ModuloViaFmod(T dividend, T divisor) noexcept
{
...
/// @note Modulo via <code>std::fmod</code> appears slower than via <code>std::trunc</code>.
/// @see ModuloViaFmod
template <typename T> inline auto ModuloViaTrunc(T dividend, T divisor) noexcept
{
...
template <class T> inline Angle GetAngle(const Vector2<T> value)
{
...
template <typename T> constexpr auto GetMagnitudeSquared(T value) noexcept
{
...
template <typename T> inline auto GetMagnitude(T value)
{
...
///
template <typename T1, typename T2> constexpr auto Dot(const T1 a, const T2 b) noexcept
{
    static_assert(std::tuple_size<T1>::value == std::tuple_size<T2>::value, "Dot only for same tuple-like sized types");
...
constexpr auto Cross(T1 a, T2 b) noexcept
{
    ...
    return Vector<OT, 3>{GetY(a) * GetZ(b) - GetZ(a) * GetY(b), GetZ(a) * GetX(b) - GetX(a) * GetZ(b),
...
template <typename T, typename U> constexpr auto Solve(const Matrix22<U> mat, const Vector2<T> b) noexcept
{
    const auto cp = Cross(get<0>(mat), get<1>(mat));
    const auto inverseCp = Real{1} / cp;
    using OutType = decltype((U{} * T{}) / cp);
    return (!AlmostZero(StripUnit(cp))) ? Vector2<OutType>{(get<1>(mat)[1] * b[0] - get<1>(mat)[0] * b[1]) * inverseCp,
                                                           (get<0>(mat)[0] * b[1] - get<0>(mat)[1] * b[0]) * inverseCp}
                                        : Vector2<OutType>{};
}

/// @brief Inverts the given value.
template <class IN_TYPE> constexpr auto Invert(const Matrix22<IN_TYPE> value) noexcept
{
    const auto cp = Cross(get<0>(value), get<1>(value));
    using OutType = decltype(get<0>(value)[0] / cp);
    const auto inverseCp = Real{1} / cp;
    return (!AlmostZero(StripUnit(cp))) ? Matrix22<OutType>{Vector2<OutType>{get<1>(get<1>(value)) * inverseCp,
...
template <typename T> constexpr T Solve22(const Mat33 &mat, const T b) noexcept
{
...
constexpr Mat33 GetSymInverse33(const Mat33 &value) noexcept
{
    ...
    return Mat33{Vec3{det * (a22 * a33 - a23 * a23), ex_y, ex_z}, Vec3{ex_y, det * (a11 * a33 - a13 * a13), ey_z},
...
template <class T> constexpr auto GetRevPerpendicular(const T vector) noexcept
{
...
template <class T> constexpr auto GetFwdPerpendicular(const T vector) noexcept
{
    // See http://mathworld.wolfram.com/PerpendicularVector.html
    return T{GetY(vector), -GetX(vector)};
}
...
template <typename T> constexpr T GetModuloNext(T value, T count) noexcept
{
    assert(value < count);
...
template <typename T> constexpr T GetModuloPrev(T value, T count) noexcept
{
    assert(value < count);
...
template <class T, typename U> constexpr Vector2<T> operator*(CheckedValue<T, U> s, UnitVec u) noexcept
{
    return Vector2<T>{u.GetX() * s, u.GetY() * s};
}

/// @brief Multiplication operator.
template <class T> constexpr Vector2<T> operator*(const T s, const UnitVec u) noexcept
{
    return Vector2<T>{u.GetX() * s, u.GetY() * s};
}

/// @brief Multiplication operator.
template <class T, typename U> constexpr Vector2<T> operator*(UnitVec u, CheckedValue<T, U> s) noexcept
{
    return Vector2<T>{u.GetX() * s, u.GetY() * s};
}

/// @brief Multiplication operator.
template <class T> constexpr Vector2<T> operator*(const UnitVec u, const T s) noexcept
{
    return Vector2<T>{u.GetX() * s, u.GetY() * s};
}
...
/// @see InverseRotate.
template <class T> constexpr auto Rotate(const Vector2<T> vector, const UnitVec &angle) noexcept
{
...
/// @see Rotate.
template <class T> constexpr auto InverseRotate(const Vector2<T> vector, const UnitVec &angle) noexcept
{
    const auto newX = (GetX(angle) * GetX(vector)) + (GetY(angle) * GetY(vector));
...
/// @see AlmostEqual.
template <class T> inline UnitVec GetUnitVector(Vector2<T> value, UnitVec fallback = UnitVec::GetDefaultFallback())
{
...
inline bool IsUnderActive(Velocity velocity, LinearVelocity linSleepTol, AngularVelocity angSleepTol) noexcept
{
