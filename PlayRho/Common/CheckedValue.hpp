...
/// @see CheckedValue.
template <typename T> struct NoOpChecker
{
...
template <typename ValueType, typename CheckerType = NoOpChecker<ValueType>> class CheckedValue
{
  public:
    ...
    constexpr CheckedValue(value_type value) noexcept(noexcept(checker_type{}(value))) : m_value{CheckerType{}(value)}
    {
    }
    ...
    template <typename U = ValueType> constexpr std::enable_if_t<std::is_pointer<U>::value, U> operator->() const
    {
        return m_value;
    }
...
constexpr auto operator==(const CheckedValue<LhsValueType, LhsCheckerType> &lhs,
                          const CheckedValue<RhsValueType, RhsCheckerType>
                              &rhs) noexcept(noexcept(std::declval<LhsValueType>() == std::declval<RhsValueType>()))
...
template <typename ValueType, typename CheckerType, typename Other>
constexpr auto operator*(const CheckedValue<ValueType, CheckerType> &lhs, const Other &rhs)
    -> std::enable_if_t<!IsMultipliable<CheckedValue<ValueType, CheckerType>, Other>::value,
...
template <typename ValueType, typename CheckerType, typename Other>
constexpr auto operator*(const Other &lhs, const CheckedValue<ValueType, CheckerType> &rhs)
    -> std::enable_if_t<!IsMultipliable<Other, CheckedValue<ValueType, CheckerType>>::value,
...
template <typename T> using DefaultCheckedValue = CheckedValue<T>;
