...
/// @see AreaDensity.
constexpr auto KilogramPerSquareMeter = PLAYRHO_UNIT(AreaDensity, boost::units::si::kilogram_per_square_meter);
...
template <class Unit>
inline auto hypot(const boost::units::quantity<Unit, Real> &x, const boost::units::quantity<Unit, Real> &y)
{
    using std::hypot;
...
template <class Unit> inline auto sqrt(const boost::units::quantity<Unit, Real> &q)
{
    using std::sqrt;
    using quantity_type = typename boost::units::root_typeof_helper<boost::units::quantity<Unit, Real>,
...
template <class Dimension, typename X,
          typename = std::enable_if_t<playrho::IsArithmetic<X>::value && !std::is_same<X, playrho::Real>::value &&
