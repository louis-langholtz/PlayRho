...
class Joint
{
  public:
    ...
    template <typename T> Joint(T arg) : m_self{std::make_unique<Model<T>>(std::move(arg))}
    {
        // Intentionally empty.
    }
    ...
    template <typename T, typename Tp = std::decay_t<T>,
              typename = std::enable_if_t<!std::is_same<Tp, Joint>::value && std::is_copy_constructible<Tp>::value>>
    ...
    template <typename T> friend auto TypeCast(const Joint *value) noexcept
    {
    ...
    friend void InitVelocity(Joint &object, std::vector<BodyConstraint> &bodies, const playrho::StepConf &step,
                             const ConstraintSolverConf &conf)
    {
        if (object.m_self)
...
/// @relatedalso Joint
template <typename T> inline auto TypeCast(const Joint &value)
{
