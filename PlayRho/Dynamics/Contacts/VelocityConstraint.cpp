...
VelocityConstraint::VelocityConstraint(Real friction, Real restitution, LinearVelocity tangentSpeed,
                                       const WorldManifold &worldManifold, BodyConstraint &bA, BodyConstraint &bB,
                                       Conf conf)
    : m_bodyA{&bA}, m_bodyB{&bB}, m_normal{worldManifold.GetNormal()}, m_friction{friction}, m_restitution{restitution},
...
VelocityConstraint::Point VelocityConstraint::GetPoint(Momentum normalImpulse, Momentum tangentImpulse, Length2 relA,
                                                       Length2 relB, Conf conf) const noexcept
{
    ...
    point.velocityBias = [&]() {
        ...
        const auto dv = GetContactRelVelocity(bodyA->GetVelocity(), relA, bodyB->GetVelocity(), relB);
...
void VelocityConstraint::AddPoint(Momentum normalImpulse, Momentum tangentImpulse, Length2 relA, Length2 relB,
                                  Conf conf)
{
    assert(m_pointCount < MaxManifoldPoints);
    m_points[m_pointCount] = GetPoint(normalImpulse * conf.dtRatio, tangentImpulse * conf.dtRatio, relA, relB, conf);
    ++m_pointCount;
}

VelocityConstraint::Conf GetRegVelocityConstraintConf(const StepConf &conf) noexcept
{
    return VelocityConstraint::Conf{conf.doWarmStart ? conf.dtRatio : 0, conf.velocityThreshold, conf.doBlocksolve};
}
