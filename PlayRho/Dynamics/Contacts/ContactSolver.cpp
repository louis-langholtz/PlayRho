...
VelocityPair GetVelocityDelta(const VelocityConstraint &vc, const Momentum2 impulses)
{
    ...
    return VelocityPair{-Velocity{invMassA * P, invRotInertiaA * LA}, +Velocity{invMassB * P, invRotInertiaB * LB}};
}
...
Optional<Momentum> BlockSolveNormalCase1(VelocityConstraint &vc, const LinearVelocity2 b_prime)
{
    //
    // Case 1: vn = 0
    //
...
Optional<Momentum> BlockSolveNormalCase2(VelocityConstraint &vc, const LinearVelocity2 b_prime)
{
    ...
    // vn2 = a21 * x1 + a22 * 0 + b2'
    //
    const auto newImpulses = Momentum2{-GetNormalMassAtPoint(vc, 0) * get<0>(b_prime), 0_Ns};
...
Optional<Momentum> BlockSolveNormalCase3(VelocityConstraint &vc, const LinearVelocity2 b_prime)
{
    ...
    //   0 = a21 * 0 + a22 * x2 + b2'
    //
    const auto newImpulses = Momentum2{0_Ns, -GetNormalMassAtPoint(vc, 1) * get<1>(b_prime)};
    const auto K = vc.GetK();
...
inline Momentum BlockSolveNormalConstraint(VelocityConstraint &vc)
{
    ...
    const auto b_prime = [=]() {
        ...
        // Compute b
        const auto b = LinearVelocity2{vn0 - vc.GetVelocityBiasAtPoint(0), vn1 - vc.GetVelocityBiasAtPoint(1)};
                                             ...
                                             ConstraintSolverConf conf)
{
    ...
    const auto solver_fn = [&](const d2::PositionSolverManifold psm, const Length2 pA, const Length2 pB) {
        ...
        const auto C =
            -std::clamp(conf.resolutionRate * (separation + conf.linearSlop), -conf.maxLinearCorrection, 0_m);
        ...
        return d2::PositionSolution{-d2::Position{invMassA * P, invRotInertiaA * LA},
                                    +d2::Position{invMassB * P, invRotInertiaB * LB}, separation};
    };
    ...
    case 1: {
        const auto psm0 =
            GetPSM(pc.manifold, 0, GetTransformation(posA, localCenterA), GetTransformation(posB, localCenterB));
    ...
    case 2: {
        ...
        if (AlmostEqual(StripUnit(psm0.m_separation), StripUnit(psm1.m_separation)))
        {
            ...
            return d2::PositionSolution{posA + s0.pos_a + s1.pos_a, posB + s0.pos_b + s1.pos_b, s0.min_separation};
        }
        if (psm0.m_separation < psm1.m_separation)
        {
            ...
            posA += s0.pos_a;
            posB += s0.pos_b;
            const auto psm1_prime =
                GetPSM(pc.manifold, 1, GetTransformation(posA, localCenterA), GetTransformation(posB, localCenterB));
