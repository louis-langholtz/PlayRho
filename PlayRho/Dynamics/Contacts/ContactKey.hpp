...
class ContactKey
{
  public:
    ...
    constexpr ContactKey(ContactCounter fp1, ContactCounter fp2) noexcept : m_ids{std::minmax(fp1, fp2)}
    {
        // Intentionally empty
    }
  ...
  private:
    ...
    std::pair<ContactCounter, ContactCounter> m_ids{static_cast<ContactCounter>(-1), static_cast<ContactCounter>(-1)};
};
...
template <> struct hash<playrho::ContactKey>
{
    /// @brief Argument type.
