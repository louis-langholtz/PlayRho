...
class VelocityConstraint
{
  public:
    ...
    VelocityConstraint(Real friction, Real restitution, LinearVelocity tangentSpeed, const WorldManifold &worldManifold,
  ...
  private:
    ...
    /// @see GetPointCount().
    void AddPoint(Momentum normalImpulse, Momentum tangentImpulse, Length2 relA, Length2 relB, Conf conf);

    /// Removes the last point added.
    void RemovePoint() noexcept;

    /// @brief Gets a point instance for the given parameters.
    Point GetPoint(Momentum normalImpulse, Momentum tangentImpulse, Length2 relA, Length2 relB,
...
{
    return InvMass22{InvMass2{get<0>(m_K), get<2>(m_K)}, InvMass2{get<2>(m_K), get<1>(m_K)}};
}

inline Mass22 VelocityConstraint::GetNormalMass() const noexcept
{
    return Mass22{Mass2{get<0>(m_normalMass), get<2>(m_normalMass)}, Mass2{get<2>(m_normalMass), get<1>(m_normalMass)}};
}
...
inline Length2 GetPointRelPosA(const VelocityConstraint &vc, VelocityConstraint::size_type index)
{
    return vc.GetPointRelPosA(index);
}

/// @brief Gets the point relative position B data.
inline Length2 GetPointRelPosB(const VelocityConstraint &vc, VelocityConstraint::size_type index)
{
