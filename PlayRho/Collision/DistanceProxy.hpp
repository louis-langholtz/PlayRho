  ...
  public:
    ...
    DistanceProxy(const DistanceProxy &copy) noexcept
        :
          ...
          m_vertices{copy.m_vertices}, m_normals{copy.m_normals},
#endif
          m_count{copy.m_count}, m_vertexRadius{copy.m_vertexRadius}
    {
    ...
    ///
    DistanceProxy(const NonNegative<Length> vertexRadius, const VertexCounter count, const Length2 *vertices,
                  const UnitVec *normals) noexcept
        :
#ifndef IMPLEMENT_DISTANCEPROXY_WITH_BUFFERS
          m_vertices{vertices}, m_normals{normals},
#endif
          m_count{count}, m_vertexRadius{vertexRadius}
    {
...
/// @see GetVertexCount().
/// @relatedalso DistanceProxy
template <class T> inline VertexCounter GetSupportIndex(const DistanceProxy &proxy, T dir) noexcept
{
