...
RayCastOutput RayCast(const AABB &aabb, const RayCastInput &input) noexcept
{
    ...
    for (auto i = decltype(pDelta.max_size()){0}; i < pDelta.max_size(); ++i)
    {
        ...
        else
        {
            ...
            if (tmin < t1)
            {
                normal = (i == 0) ? ((s < 0) ? UnitVec::GetLeft() : UnitVec::GetRight())
...
RayCastOutput RayCast(const DistanceProxy &proxy, const RayCastInput &input, const Transformation &transform) noexcept
{
    ...
    const auto transformedInput =
        RayCastInput{InverseTransform(input.p1, transform), InverseTransform(input.p2, transform), input.maxFraction};
...
bool RayCast(const DynamicTree &tree, RayCastInput input, const DynamicTreeRayCastCB &callback)
{
    ...
    while (!empty(stack))
    {
        ...
        else
        {
            ...
            const auto value = callback(leafData.body, leafData.fixture, leafData.childIndex, input);
            if (value == 0)
            {
...
bool RayCast(const World &world, const RayCastInput &input, const FixtureRayCastCB &callback)
{
    return RayCast(GetTree(world), input,
                   [&world, &callback](BodyID body, FixtureID fixture, ChildCounter index, const RayCastInput &rci) {
                       const auto output =
                           RayCast(GetChild(GetShape(world, fixture), index), rci, GetTransformation(world, body));
                       if (output.has_value())
                       {
