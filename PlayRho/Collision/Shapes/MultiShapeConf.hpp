...
class ConvexHull
{
  public:
    ...
    {
        return DistanceProxy{vertexRadius, static_cast<VertexCounter>(size(vertices)), data(vertices), data(normals)};
    }
...
{
    return lhs.friction == rhs.friction && lhs.restitution == rhs.restitution && lhs.density == rhs.density &&
