...
/// @relatedalso AABB
template <std::size_t N> constexpr bool operator==(const AABB<N> &lhs, const AABB<N> &rhs) noexcept
{
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
/// @relatedalso AABB
template <std::size_t N> inline bool operator<(const AABB<N> &lhs, const AABB<N> &rhs) noexcept
{
    return std::lexicographical_compare(cbegin(lhs.ranges), cend(lhs.ranges), cbegin(rhs.ranges), cend(rhs.ranges),
...
/// @relatedalso AABB
template <std::size_t N> constexpr bool TestOverlap(const AABB<N> &a, const AABB<N> &b) noexcept
{
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
template <std::size_t N> constexpr AABB<N> GetIntersectingAABB(const AABB<N> &a, const AABB<N> &b) noexcept
{
    auto result = AABB<N>{};
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
/// @relatedalso AABB
template <std::size_t N> constexpr Vector<Length, N> GetCenter(const AABB<N> &aabb) noexcept
{
...
/// @relatedalso AABB
template <std::size_t N> constexpr Vector<Length, N> GetDimensions(const AABB<N> &aabb) noexcept
{
    auto result = Vector<Length, N>{};
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
/// @relatedalso AABB
template <std::size_t N> constexpr Vector<Length, N> GetExtents(const AABB<N> &aabb) noexcept
{
...
/// @relatedalso AABB
template <std::size_t N> constexpr bool Contains(const AABB<N> &a, const AABB<N> &b) noexcept
{
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
/// @relatedalso AABB
template <std::size_t N> constexpr AABB<N> &Include(AABB<N> &var, const Vector<Length, N> &value) noexcept
{
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
///   the other AABB.
/// @relatedalso AABB
template <std::size_t N> constexpr AABB<N> &Include(AABB<N> &var, const AABB<N> &val) noexcept
{
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
template <std::size_t N> constexpr AABB<N> &Move(AABB<N> &var, const Vector<Length, N> value) noexcept
{
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
/// @relatedalso AABB
template <std::size_t N> constexpr AABB<N> &Fatten(AABB<N> &var, const NonNegative<Length> amount) noexcept
{
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
///   displacement amount.
/// @relatedalso AABB
template <std::size_t N> constexpr AABB<N> GetDisplacedAABB(AABB<N> aabb, const Vector<Length, N> displacement)
{
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
/// @relatedalso AABB
template <std::size_t N> constexpr AABB<N> GetFattenedAABB(AABB<N> aabb, const Length amount)
{
...
/// @relatedalso AABB
template <std::size_t N> constexpr AABB<N> GetMovedAABB(AABB<N> aabb, const Vector<Length, N> value) noexcept
{
...
/// @relatedalso AABB
template <std::size_t N> constexpr AABB<N> GetEnclosingAABB(AABB<N> a, const AABB<N> &b)
{
    return Include(a, b);
}
...
/// @relatedalso AABB
template <std::size_t N> constexpr Vector<Length, N> GetLowerBound(const AABB<N> &aabb) noexcept
{
    auto result = Vector<Length, N>{};
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
/// @relatedalso AABB
template <std::size_t N> constexpr Vector<Length, N> GetUpperBound(const AABB<N> &aabb) noexcept
{
    auto result = Vector<Length, N>{};
    for (auto i = decltype(N){0}; i < N; ++i)
    {
...
template <std::size_t N> inline ::std::ostream &operator<<(::std::ostream &os, const AABB<N> &value)
{
    os << "{";
    auto multiple = false;
...
template <> constexpr d2::AABB GetInvalid() noexcept
{
