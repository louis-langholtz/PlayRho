...
public:
    ...
    VerticalStack()
    {
        m_bulletshape = CreateShape(GetWorld(), DiskShapeConf{}.UseRadius(0.25_m).UseDensity(20_kgpm2).UseRestitution(Real(0.05)));
        const auto ground = CreateBody(GetWorld());
        Attach(GetWorld(), ground, CreateShape(GetWorld(), EdgeShapeConf{Vec2(-40.0f, 0.0f) * 1_m, Vec2(40.0f, 0.0f) * 1_m}));
        Attach(GetWorld(), ground, CreateShape(GetWorld(), EdgeShapeConf{Vec2(20.0f, 0.0f) * 1_m, Vec2(20.0f, 20.0f) * 1_m}));
        ...
        const auto shape = CreateShape(GetWorld(),
                                       PolygonShapeConf{}.UseDensity(1_kgpm2).UseFriction(Real(0.3)).SetAsBox(hdim * 1_m, hdim * 1_m));
