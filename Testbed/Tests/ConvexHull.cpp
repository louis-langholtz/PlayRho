...
public:
    static inline const auto registered =
        RegisterTest("Convex Hull", MakeUniqueTest<ConvexHullTest>);
    ...
    ConvexHullTest()
    {
        Generate();
        m_auto = false;
        RegisterForKey(GLFW_KEY_A, GLFW_PRESS, 0, "Toggle Auto Generation",
                       [&](KeyActionMods) { m_auto = !m_auto; });
        RegisterForKey(GLFW_KEY_G, GLFW_PRESS, 0, "Generate New Random Convex Hull",
                       [&](KeyActionMods) { Generate(); });
    }
    ...
    void PostStep(const Settings&, Drawer& drawer) override
    {
        ...
        drawer.DrawPolygon(begin(shape.GetVertices()), shape.GetVertexCount(),
