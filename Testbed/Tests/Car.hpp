...
public:
    Car()
    {
        ...
        RegisterForKey(GLFW_KEY_S, GLFW_PRESS, 0, "Brake.", [&](KeyActionMods) {
            SetMotorSpeed(GetWorld(), m_backSpring, 0_rpm);
        });
    ...
    void CreateCar(bool flip = false)
    {
        const auto carPosition = (m_car != InvalidBodyID)
            ? GetPosition(GetWorld(), m_car)
            : Position{Length2{0_m, 1_m}, 0_deg};
        const auto carVelocity = (m_car != InvalidBodyID)
            ? GetVelocity(GetWorld(), m_car): Velocity{};
        if (m_frontSpring != InvalidJointID) Destroy(GetWorld(), GetBodyB(GetWorld(), m_frontSpring));
        if (m_backSpring != InvalidJointID) Destroy(GetWorld(), GetBodyB(GetWorld(), m_backSpring));
        if (m_car != InvalidBodyID) Destroy(GetWorld(), m_car);
    ...
    {
        drawer.SetTranslation(Length2{GetX(GetLocation(GetWorld(), m_car)), GetY(drawer.GetTranslation())});
    }
