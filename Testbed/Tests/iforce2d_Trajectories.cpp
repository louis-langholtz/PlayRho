...
public:
    ...
    iforce2d_Trajectories() : Test(GetTestConf()), m_groundBody{CreateBody(GetWorld())}
    {
        ...
        Attach(GetWorld(), m_launcherBody,
               CreateShape(
                   GetWorld(),
                   DiskShapeConf{}.UseRadius(2_m).UseFriction(Real(0.95)).UseDensity(1_kgpm2)));
        ...
        Attach(GetWorld(), m_littleBox2,
               CreateShape(GetWorld(), DiskShapeConf{}
                                           .UseRadius(BallSize * 1_m)
                                           .UseFriction(Real(0.95))
                                           .UseDensity(1_kgpm2)));
        ...
        RegisterForKey(GLFW_KEY_A, GLFW_PRESS, 0, "Increase launch speed.",
                       [&](KeyActionMods) { m_launchSpeed *= 1.02f; });
        RegisterForKey(GLFW_KEY_S, GLFW_PRESS, 0, "Decrease launch speed.",
                       [&](KeyActionMods) { m_launchSpeed *= 0.98f; });
        RegisterForKey(GLFW_KEY_D, GLFW_PRESS, 0, "Launch computer controlled projectile.",
                       [&](KeyActionMods) {
                           SetAwake(GetWorld(), m_littleBox2);
                           SetAcceleration(GetWorld(), m_littleBox2, m_gravity,
                                           AngularAcceleration{});
                           SetVelocity(GetWorld(), m_littleBox2, Velocity{});
        ...
        RegisterForKey(GLFW_KEY_F, GLFW_PRESS, 0, "Reset computer controlled projectile.",
                       [&](KeyActionMods) {
                           SetAcceleration(GetWorld(), m_littleBox2, LinearAcceleration2{},
                                           AngularAcceleration{});
                           SetVelocity(GetWorld(), m_littleBox2, Velocity{});
                           m_firing2 = false;
                       });
        RegisterForKey(
            GLFW_KEY_M, GLFW_PRESS, 0,
            "Hold down & use left mouse button to move the computer's target",
            [&](KeyActionMods) { SetTransform(GetWorld(), m_targetBody, GetMouseWorld(), 0_rad); });
    }
    ...
    Length getMaxHeight(Length2 startingPosition, LinearVelocity2 startingVelocity)
    {
        ...
        return GetY(startingPosition) + n * GetY(stepVelocity) +
               0.5f * (n * n + n) * GetY(stepGravity);
    }
    ...
    LinearVelocity2 getComputerLaunchVelocity()
    {
        ...
        const auto verticalVelocity = calculateVerticalVelocityForHeight(
            GetY(targetLocation) - 5_m); // computer projectile starts at y = 5
        const auto startingVelocity =
            LinearVelocity2(0, verticalVelocity); // only interested in vertical here
    ...
    void PreStep(const Settings&, Drawer& drawer) override
    {
        ...
        for (auto i = 0; i < 300; ++i) { // 5 seconds, should be long enough to hit something
            const auto trajectoryPosition =
                getTrajectoryPoint(startingPosition, startingVelocity, i * 1.0f);
        ...
        drawer.DrawSegment(computerStartingPosition, Color(1, 0, 0), displayVelEndPoint,
                           Color(0, 1, 0));
