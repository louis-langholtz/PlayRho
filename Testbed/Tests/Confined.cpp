...
public:
    ...
    enum { e_columnCount = 0, e_rowCount = 0 };
    ...
    Confined()
    {
        ...
        RegisterForKey(GLFW_KEY_C, GLFW_PRESS, 0, "Create Circle",
                       [&](KeyActionMods) { CreateCircle(); });
        RegisterForKey(GLFW_KEY_B, GLFW_PRESS, 0, "Create Box",
                       [&](KeyActionMods) { CreateBox(); });
        RegisterForKey(GLFW_KEY_I, GLFW_PRESS, 0, "Impart Impulse",
                       [&](KeyActionMods) { ImpartRandomImpulses(); });
        ...
        for (const auto& b : GetBodies(GetWorld())) {
            if (GetType(GetWorld(), b) == BodyType::Dynamic) {
                ...
                const auto centerPos =
                    Length2{GetX(position), GetY(position) - (wall_length / Real{2})};
                const auto angle_from_center = GetAngle(centerPos);
                const auto direction = angle_from_center + Pi * 1_rad;
                const auto magnitude =
                    sqrt(Square(StripUnit(wall_length)) * 2) * GetMass(GetWorld(), b) * 20_mps;
