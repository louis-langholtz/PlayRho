    ...
    Tiles()
    {
            ...
            for (auto j = 0; j < M; ++j) {
                ...
                for (auto i = 0; i < N; ++i) {
                    ground.Attach(CreateShape(
                        GetWorld(),
                        PolygonShapeConf{}.SetAsBox(a * 1_m, a * 1_m, position * 1_m, 0_deg)));
                    ++m_fixtureCount;
...

        {
            ...
            const auto shape = CreateShape(
                GetWorld(), PolygonShapeConf{}.UseDensity(5_kgpm2).SetAsBox(a * 1_m, a * 1_m));
        ...
        RegisterForKey(GLFW_KEY_C, GLFW_PRESS, 0, "Make a snapshot.",
