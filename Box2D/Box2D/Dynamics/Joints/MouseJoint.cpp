...
void MouseJoint::InitVelocityConstraints(BodyConstraints& bodies, const StepConf& step, const ConstraintSolverConf&)
{
	...
	if (step.doWarmStart)
	{
		...
		velB += Velocity{m_invMassB * m_impulse, Radian * m_invIB * Cross(m_rB, m_impulse)};
	}
	else
	{
		m_impulse = Vec2_zero;
...
RealNum MouseJoint::SolveVelocityConstraints(BodyConstraints& bodies, const StepConf& step)
{
	...
	const auto Cdot = velB.linear + (GetRevPerpendicular(m_rB) * RealNum{velB.angular / Radian});
	...
	velB += Velocity{m_invMassB * deltaImpulse, Radian * m_invIB * Cross(m_rB, deltaImpulse)};
