...
static inline PositionSolverManifold GetPSM_ForFaceA(Vec2 lp, Vec2 plp, float_t totalRadius, Vec2 ln,
													 const Transform& xfA, const Transform& xfB)
{
	const auto planePoint = Mul(xfA, lp);
	const auto clipPoint = Mul(xfB, plp);
	const auto normal = Mul(xfA.q, ln);
...
static inline PositionSolverManifold GetPSM_ForFaceB(Vec2 lp, Vec2 plp, float_t totalRadius, Vec2 ln,
											   const Transform& xfA, const Transform& xfB)
{
	const auto planePoint = Mul(xfB, lp);
	const auto clipPoint = Mul(xfA, plp);
	const auto normal = Mul(xfB.q, ln);
	const auto separation = Dot(clipPoint - planePoint, normal) - totalRadius;
							  ...
							  PositionSolverManifold::index_t index)
{
	...
	{
		case Manifold::e_circles: return GetPSM_ForCircles(manifold.GetLocalPoint(), manifold.GetPoint(index).localPoint, totalRadius, xfA, xfB);
		case Manifold::e_faceA: return GetPSM_ForFaceA(manifold.GetLocalPoint(), manifold.GetPoint(index).localPoint, totalRadius, manifold.GetLocalNormal(), xfA, xfB);
		case Manifold::e_faceB: return GetPSM_ForFaceB(manifold.GetLocalPoint(), manifold.GetPoint(index).localPoint, totalRadius, manifold.GetLocalNormal(), xfA, xfB);
